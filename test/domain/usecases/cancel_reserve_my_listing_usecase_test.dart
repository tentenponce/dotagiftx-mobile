import 'package:dotagiftx_mobile/data/api/dotagiftx_auth_api.dart';
import 'package:dotagiftx_mobile/data/core/constants/api_constants.dart';
import 'package:dotagiftx_mobile/data/requests/patch_my_market_request.dart';
import 'package:dotagiftx_mobile/domain/usecases/cancel_reserve_my_listing_usecase.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mockito/annotations.dart';
import 'package:mockito/mockito.dart';

import 'cancel_reserve_my_listing_usecase_test.mocks.dart';

@GenerateNiceMocks([MockSpec<DotagiftxAuthApi>()])
void main() {
  group(CancelReserveMyListingUsecaseImpl, () {
    late MockDotagiftxAuthApi mockDotagiftxAuthApi;

    setUp(() {
      mockDotagiftxAuthApi = MockDotagiftxAuthApi();
    });

    CancelReserveMyListingUsecaseImpl createUnitToTest() {
      return CancelReserveMyListingUsecaseImpl(mockDotagiftxAuthApi);
    }

    group('cancel', () {
      const testMarketId = 'test-market-id-123';
      const testNotes = 'Test cancellation notes';

      test('should cancel reservation successfully with notes', () async {
        // Arrange
        when(
          mockDotagiftxAuthApi.patchMyMarket(
            testMarketId,
            const PatchMyMarketRequest(
              status: ApiConstants.queryMarketStatusCancelled,
              notes: testNotes,
            ),
          ),
        ).thenAnswer((_) async {});

        // Act
        await createUnitToTest().cancel(
          marketId: testMarketId,
          notes: testNotes,
        );

        // Assert
        verify(
          mockDotagiftxAuthApi.patchMyMarket(
            testMarketId,
            const PatchMyMarketRequest(
              status: ApiConstants.queryMarketStatusCancelled,
              notes: testNotes,
            ),
          ),
        ).called(1);
      });

      test('should cancel reservation successfully without notes', () async {
        // Arrange
        when(
          mockDotagiftxAuthApi.patchMyMarket(
            testMarketId,
            const PatchMyMarketRequest(
              status: ApiConstants.queryMarketStatusCancelled,
              notes: null,
            ),
          ),
        ).thenAnswer((_) async {});

        // Act
        await createUnitToTest().cancel(marketId: testMarketId);

        // Assert
        verify(
          mockDotagiftxAuthApi.patchMyMarket(
            testMarketId,
            const PatchMyMarketRequest(
              status: ApiConstants.queryMarketStatusCancelled,
              notes: null,
            ),
          ),
        ).called(1);
      });

      test('should throw exception when API call fails', () async {
        // Arrange
        final exception = Exception('API Error');
        when(
          mockDotagiftxAuthApi.patchMyMarket(
            testMarketId,
            const PatchMyMarketRequest(
              status: ApiConstants.queryMarketStatusCancelled,
              notes: testNotes,
            ),
          ),
        ).thenThrow(exception);

        // Act & Assert
        expect(
          () => createUnitToTest().cancel(
            marketId: testMarketId,
            notes: testNotes,
          ),
          throwsA(equals(exception)),
        );

        verify(
          mockDotagiftxAuthApi.patchMyMarket(
            testMarketId,
            const PatchMyMarketRequest(
              status: ApiConstants.queryMarketStatusCancelled,
              notes: testNotes,
            ),
          ),
        ).called(1);
      });
    });
  });
}
